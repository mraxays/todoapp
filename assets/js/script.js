...
new Sortable(todoList, {
    ...
    onEnd: function (evt) {
        // Get the moved item and its new position
...
function updateStats() {
    ...
    document.getElementById("progressText").innerHTML = `${Math.round(
        progress
    )}% <i class="ri-check-double-fill"></i>`;
    document.getElementById(
        "completedText"
    ).innerHTML = `${completedTasks} <i class="ri-checkbox-circle-fill"></i>`;
...
function toggleTodo(index) {
    ...
    if (todos[index].completed) {
        showNotification(
            ...
            `Great job completing: ${todos[index].text}`
        );
    }
    saveTodos();
    renderTodos();
